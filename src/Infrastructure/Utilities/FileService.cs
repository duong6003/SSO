...
namespace Infrastructure.Modules.Users.Utilities
{
    public interface IFileService : IScopedService
    {
        Task<string?> UploadFileAsync(IFormFile file, string actionName);
        Task<string?> UpdateFileAsync(IFormFile file, string actionName, string? oldPath);
        Task<bool> DeleteFileAsync(string? oldPath);
        Task<bool> DeleteFileMultipleAsync(IEnumerable<string> fileNames);
    ...
    public class FileService : IFileService
    {
        ...
        public async Task<string?> UpdateFileAsync(IFormFile file, string actionName, string? oldPath)
        {
        ...
        public async Task<bool> DeleteFileMultipleAsync(IEnumerable<string> fileNames)
        {
            (bool success, string? error) = await AmazonS3Utility.DeleteMultipleResourceAsync(Configuration["AmazonS3:BucketName"], fileNames);
            if(error is not null)
            {
                Log.Error(error);
                return false;
