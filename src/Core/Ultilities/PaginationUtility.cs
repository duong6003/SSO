...
namespace Core.Ultilities
{
    public class PaginationUtility<T> : List<T>
    {
        ...
        public PaginationUtility(IEnumerable<T> items, long count, long pageNumber, long pageSize)
        {
            Total = count;
            PageSize = pageSize;
            Current = pageNumber;
        ...
        public static PaginationUtility<T> ToPagedList(IEnumerable<T> source, int pageNumber, int pageSize)
        {
